...
async def test_get_dishes():
    async with AsyncClient(app=app, base_url="http://test") as ac:
        response = await ac.get("/api/dishes/")
...
async def test_create_dish():
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        response = await ac.post("/api/dishes/", json=new_dish)
    assert response.status_code == 200
    data = response.json()
...
async def test_delete_dish():
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        create_resp = await ac.post("/api/dishes/", json=new_dish)
        assert create_resp.status_code == 200
        dish_id = create_resp.json()["id"]

        delete_resp = await ac.delete(f"/api/dishes/{dish_id}")
        assert delete_resp.status_code == 200
        assert delete_resp.json() == {"detail": "Блюдо удалено"}

        delete_resp_again = await ac.delete(f"/api/dishes/{dish_id}")
